...
// Custom hash function for vector<int> to use in unordered_set
struct VectorHash {
    size_t operator()(const vector<int>& v) const {
        size_t hash = 0;
        for (int x : v) hash ^= hash * 31 + x;
        return hash;
    }
};
...
// Generates and evaluates a single neighbor by flipping the i-th bit
vector<int> getNeighbor(const vector<int>& solution, int i) {
    vector<int> neighbor = solution;
    neighbor[i] = 1 - neighbor[i];
    return neighbor;
}

// Runs the optimized Tabu Search algorithm
vector<int> tabuSearch(const vector<vector<int>>& TC, vector<int> initialSolution, int maxIterations, int maxWeight, int tabuTenure) {
    ...
    int bestGain = calculateGain(TC, bestSolution);
    unordered_set<vector<int>, VectorHash> tabuList; // Faster tabu list
    queue<vector<int>> tabuQueue; // To manage tenure

    int n = initialSolution.size();
    int noImprovementCount = 0;
    const int maxNoImprovement = 100; // Early termination threshold

    for (int iter = 1; iter <= maxIterations && noImprovementCount < maxNoImprovement; ++iter) {
        vector<int> bestNeighbor;
        int bestNeighborGain = -1;
        int currentGain = calculateGain(TC, currentSolution);
        int currentWeight = calculateWeight(TC, currentSolution);

        // Evaluate neighbors by flipping each bit
        for (int i = 0; i < n; ++i) {
            vector<int> neighbor = getNeighbor(currentSolution, i);
            if (tabuList.find(neighbor) == tabuList.end()) { // Not in tabu list
                // Incremental gain and weight calculation
                int deltaGain = TC[0][i] * (neighbor[i] == 1 ? 1 : -1);
                int deltaWeight = TC[1][i] * (neighbor[i] == 1 ? 1 : -1);
                int neighborGain = currentGain + deltaGain;
                int neighborWeight = currentWeight + deltaWeight;

                if (neighborWeight <= maxWeight && neighborGain > bestNeighborGain) {
                    bestNeighborGain = neighborGain;
                    bestNeighbor = neighbor;
                }
            }
        }

        // Update solutions if a valid neighbor is found
        if (!bestNeighbor.empty()) {
            currentSolution = bestNeighbor;
            if (bestNeighborGain > bestGain) {
                bestSolution = bestNeighbor;
                bestGain = bestNeighborGain;
            ...
            // Update tabu list
            tabuList.insert(currentSolution);
            tabuQueue.push(currentSolution);
